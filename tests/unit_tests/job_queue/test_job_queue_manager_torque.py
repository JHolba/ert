...
def _build_jobqueuenode(monkeypatch, dummy_config: JobConfig, job_id=0):
    monkeypatch.setattr(ert.job_queue.job_queue_node, "forward_model_ok", dummy_config["ok_callback"])
    monkeypatch.setattr(JobQueueNode, "run_exit_callback", dummy_config["exit_callback"])
...
def test_that_torque_driver_passes_options_to_qstat(
    monkeypatch, temp_working_directory, dummy_config, user_qstat_option, expected_options
):
...
def test_torque_job_status_from_qstat_output(
    monkeypatch, temp_working_directory, dummy_config, job_state, exit_status, expected_status
):
    _deploy_script(dummy_config["job_script"], SIMPLE_SCRIPT)
    _deploy_script("qsub", MOCK_QSUB)
    _deploy_script(
