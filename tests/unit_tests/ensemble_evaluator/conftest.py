...
def make_ensemble_builder(queue_config):
    def _make_ensemble_builder(monkeypatch, tmpdir, num_reals, num_jobs, job_sleep=0):
        monkeypatch.setattr(
            ert.job_queue.job_queue_node, "forward_model_ok", lambda _, _b: (LoadStatus.LOAD_SUCCESSFUL, "")
        )
        monkeypatch.setattr(JobQueueNode, "run_exit_callback", lambda _: (LoadStatus.LOAD_FAILURE, ""))
